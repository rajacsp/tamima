# Tamima: A Family of LLaMA-based LLMs focused on Tamil Language

<img src="assets/introducing_tamil_llama.png" alt="Tamima Image" width="300" height="auto">

## Description

This repository contains the code and models for "Tamima", a project focused on enhancing the performance of language models for the Tamil language. It builds upon the open-source LLaMA model, introducing additional Tamil tokens and employing the LoRA methodology for efficient training. Please read the technical report for more details.
...
- Tamima v0.2 models are out. It is a significant upgrade compared to the earlier version.
  - Tamima is now bilingual, it can fluently respond in both English and Tamil.
  - Better tokenizer.
  - Better base model.
...
| ------------------- | --------------------------- | ----------------- | --------------- | -------- | -------------- |
| Tamima 7B Base      | Base model                  | 12GB              | LLaMA 7B        | 7B       | TBD            |
| Tamima 13B Base     | Base model                  | 4GB               | LLaMA 13B       | 13B      | TBD            |
| Tamima 7B Instruct  | Instruction following model | 145k instructions | Tamima 7B Base  | 7B       | TBD            |
| Tamima 13B Instruct | Instruction following model | 145k instructions | Tamima 13B Base | 13B      | TBD            |

### Quantized Version of Available Models

| Model               | Format | Bits                 | Download Links |
| ------------------- | ------ | -------------------- | -------------- |
| Tamima 7B Base      | GGUF   | Q4_K_M, Q5_K_M, Q8_0 | TBD            |
| Tamima 13B Base     | GGUF   | Q4_K_M, Q5_K_M, Q8_0 | TBD            |
| Tamima 7B Instruct  | GGUF   | Q4_K_M, Q5_K_M, Q8_0 | TBD            |
| Tamima 13B Instruct | GGUF   | Q4_K_M, Q5_K_M, Q8_0 | TBD            |
...
> **Note:** The benchmarks test the model's capabilities in English reasoning, although the Tamima models were not trained on quality reasoning tasks in English it shows decent performance across most benchmarks.

| Model               | Average         | ARC             | HellaSwag       | MMLU            | TruthfulQA     | Winogrande      | GSM8K          |
| ------------------- | --------------- | --------------- | --------------- | --------------- | -------------- | --------------- | -------------- |
| Tamima 13B Instruct | **51.59** | **54.52** | 79.35           | 50.37           | 41.22          | **76.56** | **7.51** |
| Tamima 13B Base     | 49.5            | 52.82           | **79.95** | **52.05** | 36.56          | 75.61           | 0              |
| Tamima 7B Instruct  | 45.52           | 48.04           | 70.97           | 39.95           | **41.7** | 70.64           | 1.82           |
| Tamima 7B Base      | 44.52           | 46.67           | 72.85           | 40.95           | 35.93          | 70.72           | 0              |
...
A simple interactive demo of Tamima-7B-Instruct-v0.1 is hosted in the HuggingFace Space here -> [abhinand/tamil-llama-playground](https://huggingface.co/spaces/abhinand/tamil-llama-playground)

<img src="assets/demo_screenshot.png" alt="Tamima Image" width="75%" height="auto">
...
2. **Locate the Tamima Model**: After installation, open LM Studio and use the search bar to find the "Tamima" model. Alternatively, if you have the GGUF model ID, paste it directly into the search bar.
3. **Download the Appropriate Model Variant**: Depending on your system's specifications, select the appropriate variant of the Tamima model. Click on the 'Download' button to start the download process.
...
5. **Select and Load the Model**: Click on "Select a model to load" located on the top bar. From the list, choose the Tamima variant that you previously downloaded.
6. **Initiate Conversations with the Model**: The Tamima model is now ready to use. You can start engaging in conversations in the chat area of LM Studio.
...
2. **Download the Modelfile**: Access the GitHub repository and download the [Modelfile](config/ollama/Modelfile). This file is necessary for setting up the Tamima model in Ollama.
...
4. **Download the Tamima Model**: Execute the following command in your terminal to download the desired Tamima model from the GitHub repository:
   ...
   This command downloads the Tamima model GGUF file and saves it as `tamil-llama.gguf` in your current directory.
5. **Import and Run the Model in Ollama**: After downloading the model, use the following command to create and run the Tamima model in Ollama:

   ```bash
   ollama create tamil-llama -f Modelfile
   ```

   This command imports the Tamima model into Ollama and prepares it for use.
...
**Tamima Eval**: [abhinand/tamil-llama-eval](https://huggingface.co/datasets/abhinand/tamil-llama-eval)
...
If you use this model or the Tamima dataset in your research, please cite:
...
This project is forked from the original [Tamil-LLaMA](https://github.com/abhinand5/tamil-llama) by Abhinand Balachandran. The original repository is no longer actively maintained and the code has become outdated. We forked it to continue contributing to the development of Tamil language models.
